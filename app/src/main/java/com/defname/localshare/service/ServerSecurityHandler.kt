...
class ServerSecurityHandler(
    ...
    // Rate limiting: ip -> (failCount, windowStartMs)
    ...
    suspend fun checkLandingAccess(call: RoutingCall): LandingAccess {
        ...
        // Legacy whitelist support: in-app approve via securityRepository still works
        if (securityRepository.isWhitelisted(remoteHost)) return LandingAccess.Allowed

        val sessionId = getSessionCookie(call)
        if (sessionId != null) {
    ...
    suspend fun verifyAccess(call: RoutingCall): Boolean {
        ...
        if (securityRepository.isWhitelisted(remoteHost)) return true

        val sessionId = getSessionCookie(call) ?: return false
        val session = sessions[sessionId] ?: return false
        return session.state == SessionState.APPROVED && session.ip == remoteHost
    }

    // ─── Non-Suspending In-Flight Check (SSE heartbeat) ──────────────────────
    ...
    fun isStillAllowed(call: RoutingCall): Boolean {
        ...
        if (!securityRepository.isApprovalRequired()) return true
        if (securityRepository.isWhitelisted(remoteHost)) return true
        val sessionId = getSessionCookie(call) ?: return true // likely same-device
        val session = sessions[sessionId] ?: return false
        return session.state == SessionState.APPROVED && session.ip == remoteHost
    }
    ...
    // ─── Backwards-Compat for LocalShareService / in-app UI ─────────────────

    fun approveIp(ip: String) {
        val sessionId = getPendingSessionIdForIp(ip)
        if (sessionId != null) {
    ...
    fun blockIp(ip: String) {
        ...
        sessions.values
            .filter { it.ip == ip && it.state == SessionState.PENDING }
